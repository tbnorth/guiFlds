...
class callOn:
    ...
    def __call__(self, *args):
        ...
        if self.special == '__OPENDIR__':
            t = None
            if self.target: t = self.target.get()
            fileName = tkFileDialog.askdirectory(parent=self.ui.master, title='Select Folder', initialdir=t)
            if fileName:
                if self.target:
        ...
        if self.special == '__OPENFILE__':
            fileName = tkFileDialog.askopenfilename(parent=self.ui.master,filetypes=[('Any File','*.*')], title='Open File')
            if fileName:
                if self.target:
                    self.target.set(fileName)
                else:
        ...
        if self.special == '__SAVEFILE__':
            fileName = tkFileDialog.asksaveasfilename(parent=self.ui.master,filetypes=[('Any File','*.*')], title='Save File')
            if fileName:
                if self.target:
                    self.target.set(fileName)
                else:
    ...
    def initialize(self, master):
        ...
        for fName, fType, fDesc, fDef, fSpec in self.owner.fld_iter():
            if fType == str:
                self.entry[fName] = {}
                e = self.entry[fName]
                e['label'] = Label(self.master, text=fDesc,
                ...
                e['entry'] = Entry(self.master, relief='sunken', width='15')
                ...
                if fSpec:  # add button to select file / directory in this entry
                    x += ew + px
                    e['helper'] = Button(self.master,text=' .. ',
                                         justify="center", relief="raised",
                                         width="15")
                    e['helper'].place(x=x, y=y, width=fbw, height=bh)
                    e['helper'].bind("<ButtonRelease-1>",
                                     callOn(self, self.owner, fName,
                                            special = fSpec, target=e['value']))
                y += py + lh
                x = px
        ...
        for fName, fType, fDesc, fDef, fSpec in self.owner.fld_iter():
            if fType == bool and not fSpec:
                self.entry[fName] = {}
                e = self.entry[fName]
                e['entry'] = Checkbutton(self.master, text=fDesc,
                                         justify="left", relief="flat",
                                         width="15")
                e['entry'].place(x=x, y=y, width=cw, height=lh)
                e['value'] = StringVar()
                e['value'].set(self.owner.fldDict()[fName])
                e['entry'].configure(variable=e['value'], onvalue="yes",
                                     offvalue="no")
                
                e['entry'].bind("<ButtonRelease-1>", 
                                lambda e: self.readFlds())
                x += cw + px
                r += 1
                if r == cr:
                    r = 0
        ...
        for fName, fType, fDesc, fDef, fSpec in self.owner.fld_iter():
            if fType == bool and fSpec == '__RADIO__':
                ...
                e['entry'] = Radiobutton(self.master, text=fDesc,
                                         justify="left", relief="flat",
                                         width="15", value=fDef)
                e['entry'].place(x=x, y=y, width=cw, height=lh)
                sv = self.radio.setdefault(fName, StringVar())
        ...
        # action buttons
        for fName, fType, fDesc, fDef, fSpec in self.owner.fld_iter():
            if fType == None and '-->' not in fDesc:
                self.entry[fName] = {}
                e = self.entry[fName]
                e['button'] = Button(self.master,text=fDesc,
                                     justify="center", relief="raised",
                                     width="15")
                e['button'].place(x=x, y=y, width=cw, height=bh)
                e['button'].bind("<ButtonRelease-1>",
                                 callOn(self, self.owner, fName,
                                        special = fSpec))
        ...
        if menuName:
            ...
            for i, s, n in menuList:
                top_File.add("command", label = i,
                             command = callOn(self, self.owner, n, s))
