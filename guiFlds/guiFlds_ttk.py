...
class guiFldsTTK:
    ...
    def initialize(self, master):
        ...
        self.initComplete = 0
        frame = ttk.Frame(master, width=w, height=h)
        frame.pack()
        self.master = master
        ...
        self.style.configure(
            "R.TLabel", justify="right", anchor="e", height=lh, width=lw
        )
        for fName, fType, fDesc, fDef, fSpec in self.owner.fld_iter():
            if fType == str:
                ...
                e["label"]["justify"] = "right"

                e["label"].place(x=x, y=y, width=lw, height=lh)

                x += lw + px

                e["entry"] = ttk.Entry(self.master, width="15")
                ...
                if fSpec:  # add button to select file / directory in this entry
                    x += ew + px
                    e["helper"] = ttk.Button(self.master, text=" .. ", width="15")
        ...
        for fName, fType, fDesc, fDef, fSpec in self.owner.fld_iter():
            if fType == bool and not fSpec:
                self.entry[fName] = {}
                e = self.entry[fName]
                e["entry"] = ttk.Checkbutton(self.master, text=fDesc, width="15")
        ...
        for fName, fType, fDesc, fDef, fSpec in self.owner.fld_iter():
            if fType == bool and fSpec == "__RADIO__":
                ...
                e["entry"] = ttk.Radiobutton(
                    self.master, text=fDesc, width="15", value=fDef
                )
                e["entry"].place(x=x, y=y, width=cw, height=lh)
        ...
        if menuName:
            self.menuBar = tk.Menu(master, bd=2)
        ...
        y += py
        lbframe = ttk.Frame(self.master)
        self.Text_1_frame = lbframe
        scrollbar = ttk.Scrollbar(lbframe, orient=tk.VERTICAL)
        self.Text_1 = tk.Text(
